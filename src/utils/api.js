...
export const simplifyApi = {
  // Check if user has connected Simplify account
  checkConnection: (userId) => {
    return apiRequest(`/api/simplify/user-session/${userId}`);
  },

  // Initiate Simplify login process
  initiateLogin: (username) => {
    return apiRequest('/api/simplify/initiate-login', {
      method: 'POST',
      body: JSON.stringify({ username })
    });
  },

  // Make API requests to Simplify Jobs
  makeRequest: (endpoint, method = 'GET', data = null) => {
    return apiRequest('/api/simplify/api-request', {
      method: 'POST',
      body: JSON.stringify({
        endpoint,
        method,
  ...
  // Apply to a job on Simplify
  applyToJob: (jobId, applicationData = {}) => {
    return simplifyApi.makeRequest(`/jobs/${jobId}/apply`, 'POST', applicationData);
  },

  // Get user's Simplify profile
  getProfile: () => {
    return simplifyApi.makeRequest('/profile');
  },

  // Get jobs from Simplify
  getJobs: (params = {}) => {
    const queryParams = new URLSearchParams(params).toString();
    return simplifyApi.makeRequest(`/jobs?${queryParams}`);
  }
};